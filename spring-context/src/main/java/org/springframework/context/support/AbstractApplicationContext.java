...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public ConfigurableEnvironment getEnvironment() {
		// 判断对应的环境对象是不是null 如果是null那么就创建一个默认的环境变量对象
	...
	protected ConfigurableEnvironment createEnvironment() {
		// 默认返回的对象是StandardEnvironment的实例
	...
	protected ResourcePatternResolver getResourcePatternResolver() {
		// 返回一个PathMatchingResourcePatternResolver对象
	...
	public void refresh() throws BeansException, IllegalStateException {
		// 首先加一个锁  防止在refresh()方法没有结束前再次进行刷新
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 调用 BeanFactoryPostProcessor 各个实现类的 postProcessBeanFactory(beanFactory)方法
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				// 注册 BeanPostProcessor
				// 此接口的两个方法 postProcessBeforeInitialization 和 postProcessAfterInitialization
				// 这两个方法在 Bean 的初始化之前和初始化之后得到执行 {这个时候 Bean 还没有完成初始化}
	...
	protected void prepareRefresh() {
		...
		if (this.earlyApplicationListeners == null) {
			// 如果没有 那么就按照当前已经初始化的监听器初始化在刷新容器之前的监听器
			this.earlyApplicationListeners = new LinkedHashSet<>(this.applicationListeners);
		}
		else {
			// Reset local application listeners to pre-refresh state.
			// 如果有先前初始化的监听器 那么就先清理当前的 然后直接使用刷新容器之前已经初始化好的监听器
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 关闭旧的 BeanFactory (如果有的话) 创建新的 BeanFactory 加载 Bean 定义、注册 Bean 等等
		refreshBeanFactory();
	...
	protected void registerListeners() {
		...
		// 先将applicationContext对象中的listener放到广播器里面
		...
		// 从beanFactory中获取ApplicationListener类型的事件监听器
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 类似于锁定当前beanFactory中的beanDefinition对象
